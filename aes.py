...
# PKCS5 padding
BS = 16
pad = lambda s: s + (BS - len(s) % BS) * chr(BS - len(s) % BS).encode('utf-8') if isinstance(s, bytes) else s + (BS - len(s) % BS) * chr(BS - len(s) % BS)
unpad = lambda s : s[0:-s[-1]] if isinstance(s, bytes) else s[0:-ord(s[-1])]
